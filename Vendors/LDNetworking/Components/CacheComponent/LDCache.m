...
- (NSCache *)cache {
  if (_cache == nil) {
    ...
    _cache.countLimit = kLDCacheCountLimit;
  }
  return _cache;
}
...
+ (instancetype)sharedInstance {
  ...
  static LDCache *sharedInstance;
  dispatch_once(&onceToken, ^{
    sharedInstance = [[LDCache alloc] init];
  });
...
- (NSData *)fetchCachedDataWithServiceIdentifier:(NSString *)serviceIdentifier
  ...
  return [self
      fetchCachedDataWithKey:[self keyWithServiceIdentifier:serviceIdentifier
...
- (NSData *)fetchCachedDataWithKey:(NSString *)key {
  LDCachedObject *cachedObject = [self.cache objectForKey:key];
...
- (void)saveCacheWithData:(NSData *)cachedData key:(NSString *)key {
  LDCachedObject *cachedObject = [self.cache objectForKey:key];
  if (cachedObject == nil) {
    cachedObject = [[LDCachedObject alloc] init];
  }
...
- (NSString *)keyWithServiceIdentifier:(NSString *)serviceIdentifier
                         ...
                         requestParams:(NSDictionary *)requestParams {
  return [NSString
      stringWithFormat:@"%@%@%@", serviceIdentifier, methodName,
                       [requestParams LD_urlParamsStringSignature:NO]];
}
