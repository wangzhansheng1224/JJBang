...
@class LDAPIBaseManager;
// 在调用成功之后的params字典里面，用这个key可以取出requestID
static NSString *const kLDAPIBaseManagerRequestID =
    @"kLDAPIBaseManagerRequestID";
...
@protocol LDAPIManagerParamSourceDelegate <NSObject>
@required
- (NSDictionary *)paramsForApi:(LDAPIBaseManager *)manager;
@end

@protocol LDAPIManagerValidator <NSObject>
@required
/*
 ...
 */
typedef NS_ENUM(NSUInteger, LDAPIManagerErrorType) {
  LDAPIManagerErrorTypeDefault, //没有产生过API请求，这个是manager的默认状态。
  LDAPIManagerErrorTypeSuccess,      // API请求成功且返回数据正确，此时manager的数据是可以直接拿来使用的。
  LDAPIManagerErrorTypeNoContent,    // API请求成功但返回数据不正确。如果回调数据验证函数返回值为NO，manager的状态就会是这个。
  LDAPIManagerErrorTypeParamsError,  //参数错误，此时manager不会调用API，因为参数验证是在调用API之前做的。
  LDAPIManagerErrorTypeTimeout,      //请求超时。RTApiProxy设置的是20秒超时，具体超时时间的设置请自己去看RTApiProxy的相关代码。
  LDAPIManagerErrorTypeNoNetWork,    //网络不通。在调用API之前会判断一下当前网络是否通畅，这个也是在调用API之前验证的，和上面超时的状态是有区别的。
  LDAPIManagerErrorTypeUnauthorized, //验证失败。
  LDAPIManagerErrorTypeNotFound,     //找不到action。
  LDAPIManagerErrorTypeServiceError, //服务器返回错误提示
};

typedef NS_ENUM(NSUInteger, LDAPIManagerRequestType) {
  LDAPIManagerRequestTypeGet,
  LDAPIManagerRequestTypePost,
  LDAPIManagerRequestTypeRestGet,
  LDAPIManagerRequestTypeRestPost
};

/*
 RTAPIBaseManager的派生类必须符合这些protocal
 */
@protocol LDAPIManagerInterceptor <NSObject>

@optional
- (void)manager:(LDAPIBaseManager *)manager
    beforePerformSuccessWithResponse:(LDURLResponse *)response;
- (void)manager:(LDAPIBaseManager *)manager
    afterPerformSuccessWithResponse:(LDURLResponse *)response;

- (void)manager:(LDAPIBaseManager *)manager
    beforePerformFailWithResponse:(LDURLResponse *)response;
- (void)manager:(LDAPIBaseManager *)manager
    afterPerformFailWithResponse:(LDURLResponse *)response;
...
/*
 RTAPIBaseManager的派生类必须符合这些protocal
 */
@protocol LDAPIManager <NSObject>

@required
- (NSString *)methodName;
- (NSString *)serviceType;
- (LDAPIManagerRequestType)requestType;
...
@interface LDAPIBaseManager : NSObject
@property(nonatomic, weak) id<LDAPIManagerApiCallBackDelegate> delegate;
@property(nonatomic, weak) id<LDAPIManagerParamSourceDelegate> paramSource;
@property(nonatomic, weak) id<LDAPIManagerValidator> validator;
//里面会调用到NSObject的方法，所以这里不用id
@property(nonatomic, weak) NSObject<LDAPIManager> *child;

@property(nonatomic, weak) id<LDAPIManagerInterceptor> interceptor;
...
@property(nonatomic, readonly) LDAPIManagerErrorType errorType;
...
/*
 ...
 LDAPIBaseManager会先调用这个函数，然后才会调用到 id<LDAPIManagerValidator> 中的
 manager:isCorrectWithParamsData:
