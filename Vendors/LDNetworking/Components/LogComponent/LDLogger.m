...
@property(nonatomic, strong, readwrite) LDLoggerConfiguration *configParams;

@end

@implementation LDLogger
...
+ (void)logDebugInfoWithRequest:(NSURLRequest *)request
  ...
  if ([service respondsToSelector:@selector(isOnline)]) {
    NSInvocation *invocation = [NSInvocation
        invocationWithMethodSignature:
            [service methodSignatureForSelector:@selector(isOnline)]];
  ...
  [logString
      appendFormat:@"API Name:\t\t%@\n", [apiName LD_defaultValue:@"N/A"]];
  [logString
      appendFormat:@"Method:\t\t\t%@\n", [httpMethod LD_defaultValue:@"N/A"]];
  [logString appendFormat:@"Version:\t\t%@\n",
                          [service.apiVersion LD_defaultValue:@"N/A"]];
  [logString appendFormat:@"Service:\t\t%@\n", [service class]];
  [logString
      appendFormat:@"Status:\t\t\t%@\n", isOnline ? @"online" : @"offline"];
  [logString appendFormat:@"Public Key:\t\t%@\n",
                          [service.publicKey LD_defaultValue:@"N/A"]];
  [logString appendFormat:@"Private Key:\t%@\n",
                          [service.privateKey LD_defaultValue:@"N/A"]];
...
+ (void)logDebugInfoWithResponse:(NSHTTPURLResponse *)response
  ...
  if (shouldLogError) {
    ...
    [logString
        appendFormat:@"Error Domain Code:\t\t\t\t\t\t%ld\n", (long)error.code];
...
+ (void)logDebugInfoWithCachedResponse:(LDURLResponse *)response
  ...
  [logString
      appendFormat:@"API Name:\t\t%@\n", [methodName LD_defaultValue:@"N/A"]];
  [logString appendFormat:@"Version:\t\t%@\n",
                          [service.apiVersion LD_defaultValue:@"N/A"]];
  [logString appendFormat:@"Service:\t\t%@\n", [service class]];
  [logString appendFormat:@"Public Key:\t\t%@\n",
...
+ (instancetype)sharedInstance {
  ...
  static LDLogger *sharedInstance;
...
- (instancetype)init {
  self = [super init];
  if (self) {
    self.configParams = [[LDLoggerConfiguration alloc] init];
...
- (void)logWithActionCode:(NSString *)actionCode params:(NSDictionary *)params {
  ...
  [actionDict addEntriesFromDictionary:[LDCommonParamsGenerator
                                               commonParamsDictionaryForLog]];
  NSDictionary *logJsonDict = @{
    self.configParams.sendActionKey : [@[ actionDict ] LD_jsonString]
  };
  [[LDApiProxy sharedInstance]
      callPOSTWithParams:logJsonDict
