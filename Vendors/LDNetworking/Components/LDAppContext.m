...
- (NSString *)cv {
  if (_cv == nil) {
    _cv = [[[[NSBundle mainBundle] infoDictionary]
        objectForKey:@"CFBundleVersion"] LD_defaultValue:@""];
  }
  return _cv;
}
...
- (NSString *)macid {
  if (_macid == nil) {
    _macid = [[self.device LD_macaddressMD5] LD_defaultValue:@""];
  }
  return _macid;
}
...
- (NSString *)uuid {
  if (_uuid == nil) {
    _uuid = [[self.device LD_uuid] LD_defaultValue:@""];
  }
  return _uuid;
}
...
- (NSString *)ostype2 {
  if (_ostype2 == nil) {
    _ostype2 = [self.device.LD_ostype LD_defaultValue:@""];
  }
  return _ostype2;
}
...
- (NSString *)uuid2 {
  if (_uuid2 == nil) {
    _uuid2 = [self.device.LD_uuid LD_defaultValue:@""];
  }
  return _uuid2;
}
...
- (NSString *)udid2 {
  if (_udid2 == nil) {
    _udid2 = [self.device.LD_uuid LD_defaultValue:@""];
  }
  return _udid2;
}
  ...
  if (_guid == nil) {
    NSString *path =
        [[NSHomeDirectory() stringByAppendingPathComponent:@"Documents"]
            stringByAppendingPathComponent:@"RTGuid.string"];
    if ([[NSFileManager defaultManager] fileExistsAtPath:path]) {
      _guid = [[NSString stringWithContentsOfFile:path
...
- (NSString *)net {
  if (_net == nil) {
    _net = @"";
    if ([AFNetworkReachabilityManager sharedManager]
...
- (NSString *)pmodel {
  if (_pmodel == nil) {
    _pmodel = [[UIDevice currentDevice] LD_machineType];
  }
  return _pmodel;
}
...
+ (instancetype)sharedInstance {
  static LDAppContext *sharedInstance;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    sharedInstance = [[LDAppContext alloc] init];
